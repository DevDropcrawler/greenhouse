...
class GetThisMonthCropsUseCaseTest {
    ...
    @Test
    fun groupsCropsByCurrentMonthBuckets() = runBlocking {
        val crops = listOf(
            crop(1, "Basil", sow = listOf(4, 5), plant = listOf(5, 6)),
            crop(2, "Tomato", plant = listOf(4)),
            crop(3, "Lemon", harvest = listOf(4, 5, 6)),
            crop(4, "Garlic", sow = listOf(6))
        )
        val useCase = GetThisMonthCropsUseCase(FakeCropDao(crops)) { LocalDate.of(2026, 4, 26) }

        val result = useCase().first()

        assertEquals(4, result.month)
        assertEquals(listOf("Basil"), result.sow.map { it.commonName })
        assertEquals(listOf("Tomato"), result.transplant.map { it.commonName })
        assertEquals(listOf("Lemon"), result.harvest.map { it.commonName })
        assertTrue(result.sow.none { it.commonName == "Garlic" })
    }
}
